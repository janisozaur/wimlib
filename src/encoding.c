...
/*
 * Decodes the next Unicode codepoint from the string at @in, which has
 * @remaining >= 1 bytes remaining, and returns the number of bytes consumed and
...
/* Encode the Unicode codepoint @c and return the number of bytes consumed  */
...
utf8_decode_codepoint(const u8 *in, size_t remaining, bool validate, u32 *c_ret)
{
	...
	if (in[0] < 0xE0) { /* U+80...U+7FF */
		VALIDATE(remaining >= 2 && in[0] >= 0xC2 &&
			 IS_UTF8_TAIL(in[1]));
	...
	/* U+10000...U+10FFFF */
	VALIDATE(remaining >= 4 && in[0] < 0xF8 && IS_UTF8_TAIL(in[1]) &&
			 ...
			 u32 *c_ret)
{
	...
	c = get_unaligned_le16(in);
	if (unlikely(IS_SURROGATE(c))) {
		/* U+10000...U+10FFFF */
		VALIDATE(IS_LOW_SURROGATE(c) && remaining >= 4);
		c_high = get_unaligned_le16(in + 2);
		VALIDATE(IS_HIGH_SURROGATE(c_high));
		*c_ret = 0x10000 + (((c - 0xD800) << 10) | (c_high - 0xDC00));
		return 4;
	}
	*c_ret = c;
	return 2;
...
static inline int
convert(const u8 *in, size_t in_nbytes, u8 **out_ret, size_t *out_nbytes_ret,
	int ilseq_err,
	decode_codepoint_fn decode_codepoint,
	encode_codepoint_fn encode_codepoint)
{
	const u8 *in_end = in + in_nbytes;
	const u8 *p_in;
	u8 *p_out;
	size_t out_nbytes = 0;
	u8 *out;
