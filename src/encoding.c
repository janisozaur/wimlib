...
/*
 * To deal with the realities of filenames on UNIX and Windows, which are
 * defined as sequences of 8-bit units or 16-bit integers respectively and are
 * not guaranteed to be valid UTF-8 or UTF-16LE respectively, our "Unicode"
 * codecs implement some nonstandard behavior:
 *
 * 1. Each byte 'b' in an 8-bit "UTF-8" string that cannot be interpreted as
 *    UTF-8 is mapped to the surrogate codepoint 'U+D8C0 + b'.  Such codepoints
 *    are not permitted in valid Unicode.
 *
 * 2. Other surrogate codepoints (U+D800...D+FFFF) are permitted.  Again, such
 *    codepoints are not permitted in valid Unicode.
 *
 * 3. "Non-character" codepoints such as U+FFFE and U+FFFF are permitted.
 *
 * When our nonstandard codecs work with valid Unicode, their behavior is the
 * same as the standard codecs.  But when they work with "strings" that are not
 * valid Unicode, they will always perform lossless conversion from 8 bit
 * "strings" to 16 bit "strings" and vice versa.  This allows filenames to be
 * preserved and processed correctly across platforms.
 *
 * Note: this does address supporting other 8-bit encodings such as ISO-8859-1,
 * where "properly" means creating the correct Unicode (a round trip on the same
 * filesystem will still work as expected).  Currently, only UTF-8 is fully
 * supported.
 */
...
#define IS_SURROGATE_ESCAPE(c)	((c) >= 0xDC80 && (c) < 0xDD00)
 ...
 * write the decoded codepoint to *c_ret.  On invalid input the function
 * consumes at least one byte and sets *c_ret to INVALID_CODEPOINT.
 */
...
utf8_decode_codepoint(const u8 *in, size_t remaining, u32 *c_ret)
{
	...
	if (in[0] < 0xF0) {
		/* Officially, U+800...U+FFFD excluding surrogates.
		 * But we allow U+800...U+FFFF excluding surrogate escapes. */
		...
		*c_ret = ((u32)(in[0] & 0x0F) << 12) |
		...
		VALIDATE(!IS_SURROGATE_ESCAPE(*c_ret));
	#else
		VALIDATE(!IS_SURROGATE(*c_ret) && *c_ret <= 0xFFFD);
	#endif
		return 3;
	}
...
static inline unsigned
utf16le_decode_codepoint(const u8 *in, size_t remaining, u32 *c_ret)
{
	u32 h, l;
	       ...
	       encode_codepoint_fn encode_codepoint)
{
	...
	for (p_in = in; p_in != in_end; ) {
		p_in += (*decode_codepoint)(p_in, in_end - p_in, &c);
