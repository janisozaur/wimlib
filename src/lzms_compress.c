...
struct lzms_adaptive_state {
	...
	u32 recent_lz_offsets[LZMS_NUM_LZ_REPS];
	...
	u32 recent_delta_pairs[LZMS_NUM_DELTA_REPS];
...
{
	for (int i = 0; i < LZMS_NUM_LZ_REPS; i++)
		state->recent_lz_offsets[i] = i + 1;

	for (int i = 0; i < LZMS_NUM_DELTA_REPS; i++)
	...
	for (;;) {
		...
		if (num_matches) {
			...
			if (best_len >= c->mf.nice_match_len) {
				...
				for (int i = LZMS_NUM_LZ_REPS - 1; i >= 1; i--)
					cur_node->state.recent_lz_offsets[i] = cur_node->state.recent_lz_offsets[i - 1];
				cur_node->state.recent_lz_offsets[0] = offset;
				lzms_update_main_state(&cur_node->state, 1);
				lzms_update_match_state(&cur_node->state, 0);
		    ...
		    likely(in_end - in_next >= NBYTES_HASHED_FOR_DELTA + 1))
		{
			...
			for (u32 power = 0; power < NUM_POWERS_TO_CONSIDER; power++) {
				...
				if (len >= c->mf.nice_match_len) {
					...
					for (int i = LZMS_NUM_DELTA_REPS - 1; i >= 1; i--)
						cur_node->state.recent_delta_pairs[i] = cur_node->state.recent_delta_pairs[i - 1];
					cur_node->state.recent_delta_pairs[0] = pair;
					lzms_update_main_state(&cur_node->state, 1);
					lzms_update_match_state(&cur_node->state, 1);
		...
		if (cur_and_lit_cost < (cur_node + 1)->cost) {
			...
			/* try lit + LZ-rep0  */
			const u32 offset = cur_node->state.recent_lz_offsets[0];
			...
			if (length > 1) {
				...
				if (source & DELTA_SOURCE_TAG) {
					...
					if (source >= LZMS_NUM_DELTA_REPS) {
						...
						cur_node->state.recent_delta_pairs[0] = source - (LZMS_NUM_DELTA_REPS - 1);
					} else {
						/* Repeat offset delta match  */
						int rep_idx = source;
						...
						u32 pair = cur_node->state.recent_delta_pairs[rep_idx];
						for (int i = rep_idx; i >= 1; i--)
							cur_node->state.recent_delta_pairs[i] =
								cur_node->state.recent_delta_pairs[i - 1];
						cur_node->state.recent_delta_pairs[0] = pair;
					...
					if (source >= LZMS_NUM_LZ_REPS) {
						...
						cur_node->state.recent_lz_offsets[0] = source - (LZMS_NUM_LZ_REPS - 1);
					} else {
						/* Repeat offset LZ match  */
						int rep_idx = source;
						...
						u32 offset = cur_node->state.recent_lz_offsets[rep_idx];
						for (int i = rep_idx; i >= 1; i--)
							cur_node->state.recent_lz_offsets[i] =
								cur_node->state.recent_lz_offsets[i - 1];
						cur_node->state.recent_lz_offsets[0] = offset;
