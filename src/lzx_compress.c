...
{
	return max_bufsize <= min(65536,
			ARRAY_LEN(((struct lzx_compressor *)0)->offset_slot_tab_1));
}

/*
 * The following macros call either the 16-bit or the 32-bit version of a
 * matchfinder function based on the value of 'is_16_bit', which should be known
 * at compilation time.
 */
...
lzx_finish_sequence(struct lzx_sequence *last_seq, u32 litrunlen)
{
	...
	last_seq->adjusted_offset__and_match_hdr = 0x80000000;
}
...
lzx_record_item_list(struct lzx_compressor *c, u32 block_size, bool is_16_bit)
{
	...
	/* Special value to mark last sequence  */
	c->chosen_sequences[seq_idx].adjusted_offset__and_match_hdr = 0x80000000;
