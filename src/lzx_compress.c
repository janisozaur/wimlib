 ...
 *    bytes starting at the SOFT_MAX_BLOCK_SIZE'th byte.
 *
 *  - The lazy compressor may choose a sequence of literals starting at the
 *    SOFT_MAX_BLOCK_SIZE'th byte when it sees a sequence of increasingly better
 ...
 * single bit has a cost of (1 << COST_SHIFT).
 *
...
struct lzx_compressor {
	...
	/* The log base 2 of the window size for LZ match offset encoding
...
static bool
lzx_end_block_check(struct lzx_block_split_stats *stats)
{
		     ...
		     const u8 *in_block_begin, const u8 *in_next, const u8 *in_end)
{
	...
	return lzx_end_block_check(stats);
}
...
#define QUEUE(node) \
	(*(struct lzx_lru_queue *)((char *)queues + \
			((uintptr_t)(node) % (ARRAY_LEN(queues) * sizeof(*(node))))))
...
#define MATCH_BEFORE_GAP(node) \
	 ...
	 *
	 * This statement also intentionally sets the 'item' of the first node,
	 * which would otherwise have no meaning, to 0xFFFFFFFF for use as a
	 * sentinel.  See lzx_walk_item_list().
	 */
		...
		rep2_done:
			...
			for (;;) {
				...
				if (++cache_ptr == end_matches) {
					 ...
					 * + lit + rep0. */
					s32 remaining = (block_end - in_next) - next_len;
					if (likely(remaining >= 2)) {
						...
						if (load_u16_unaligned(strptr) == load_u16_unaligned(matchptr)) {
							...
							u32 limit = min(remaining,
									min(ARRAY_LEN(queues) - LZX_MAX_MATCH_LEN - 2,
									    LZX_MAX_MATCH_LEN));
							u32 rep0_len = lz_extend(strptr, matchptr, 2, limit);
							u8 lit = strptr[-1];
	...
	done_matches:
		...
		#if CONSIDER_GAP_MATCHES
			s32 offset_data = (s32)cur_node->item >> OPTIMUM_OFFSET_SHIFT;
...
lzx_compute_match_costs(struct lzx_compressor *c)
{
	...
	for (unsigned offset_slot = 0; offset_slot < num_offset_slots;
	     offset_slot++)
	{
		u32 extra_cost = (u32)lzx_extra_offset_bits[offset_slot] <<
								COST_SHIFT;
	...
	#if CONSIDER_ALIGNED_COSTS
		if (offset_slot >= 8)
			extra_cost -= LZX_NUM_ALIGNED_OFFSET_BITS << COST_SHIFT;
	#endif
...
lzx_set_default_costs(struct lzx_compressor *c, const u8 *block, u32 block_size)
{
	...
	/* The costs below are hard coded to use a COST_SHIFT of 6.  */
	...
	for (i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++)
		c->costs.aligned[i] = LZX_NUM_ALIGNED_OFFSET_BITS << COST_SHIFT;
#endif
...
lzx_set_costs_from_codes(struct lzx_compressor *c)
{
	...
	for (i = 0; i < c->num_main_syms; i++) {
		c->costs.main[i] = (lens->main[i] ? lens->main[i] :
				    MAIN_CODEWORD_LIMIT) << COST_SHIFT;
	}

	for (i = 0; i < LZX_LENCODE_NUM_SYMBOLS; i++) {
		c->costs.len[i] = (lens->len[i] ? lens->len[i] :
				   LENGTH_CODEWORD_LIMIT) << COST_SHIFT;
	}

#if CONSIDER_ALIGNED_COSTS
	for (i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++) {
		c->costs.aligned[i] = (lens->aligned[i] ? lens->aligned[i] :
				       ALIGNED_CODEWORD_LIMIT) << COST_SHIFT;
	}
#endif
 ...
 * for the block uses default costs.  Additional passes use costs taken from the
 * Huffman codes computed in the previous pass.
 */
static inline struct lzx_lru_queue
			  ...
			  bool is_16_bit)
{
	...
	resume_matchfinding:
		do {
			if (in_next >= next_search_pos) {
				...
				if (in_next >= next_observation) {
					if (best_len >= 4) {
						lzx_observe_match(&c->split_stats,
								  best_len);
						next_observation = in_next + best_len;
					} else {
						lzx_observe_literal(&c->split_stats,
								    *in_next);
		    ...
		    NUM_OBSERVATIONS_PER_BLOCK_CHECK)
		{
			...
			else if (lzx_end_block_check(&c->split_stats))
				goto end_block;
		}

		/* It's not time to end the block yet. Compute the next pause
		 * point and resume the matchfinding. */
		next_pause_point =
		  ...
		  struct lzx_output_bitstream * restrict os, bool is_16_bit)
{
	...
	do {
		...
		choose_cur_match:
			 ...
			 * remaining bytes. */
			lzx_choose_match(c, cur_len, cur_offset_data,
			...
			/* Keep going until it's time to end the block. */
			 ...
			 !lzx_should_end_block(&c->split_stats, in_block_begin,
					       in_next, in_end));
	     ...
	     void *restrict out, size_t out_nbytes_avail, void *restrict _c)
{
	...
	/* Copy the input data into the internal buffer if needed. */
	if (!c->destructive) {
