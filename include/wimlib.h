 ...
 * encoding is meant to be UTF-8.  However, for compatibility with filenames
 * that are not valid Unicode, it is not guaranteed or required that the strings
 * actually be valid Unicode.
 *
 * On Windows, each ::wimlib_tchar is a 2-byte <tt>wchar_t</tt>.  The encoding
 * is meant to be UTF-16LE.  However, unpaired surrogate and non-character
 * codepoints are permitted because neither Windows nor the NTFS filesystem
 * forbids them in filenames.
 *
 * There is an internal mechanism for losslessly translating between invalid
 * "UTF-8" filenames and invalid "UTF-16LE" filenames.  This means that all
 * possible Windows filenames can be processed on UNIX-like systems using the
 * native character type and vice versa.  Please note that this feature exists
 * for compatibility purposes, and the use of non-Unicode filenames is to be
 * strongly discouraged in general.
 *
