...
static const unsigned dec32table[] = {4, 1, 2, 1, 4, 4, 4, 4};
	...
	u32 min_length)
{
	...
	if (!UNALIGNED_ACCESS_IS_FAST || unlikely(out_end - end < WORDBYTES - 1)) {
		if (min_length >= 2)
			*out_next++ = *src++;
		if (min_length >= 3)
			*out_next++ = *src++;
	...
	if (unlikely(offset < 8)) {
		...
		store_u32_unaligned(load_u32_unaligned(src), out_next + 4);
		src -= dec64;
	} else {
